...
class InteractionGraphBuilder extends UMLSwitch<Node> {		
	...
	Map<EObject, NodeImpl> nodeCache = new HashMap<EObject, NodeImpl>(); // TODO: -> Move to the GraphImpl
	...
	public InteractionGraph build() {
		...
		activeLifelineGroups = new HashMap<Lifeline, ClusterImpl>();
	...
	public NodeImpl caseExecutionSpecification(ExecutionSpecification element) {
		...
		NodeImpl startNode = (NodeImpl)nodeCache.get(start);
	...
	public Node caseMessage(Message element) {
		...
		boolean isExecutionSpecification = recvEvent instanceof OccurrenceSpecification &&
				startExecutionSpecification.containsKey((OccurrenceSpecification)recvEvent);
		
		NodeImpl sendNode = (NodeImpl)nodeCache.get(sendEvent);
		NodeImpl recvNode = (NodeImpl)nodeCache.get(recvEvent);
