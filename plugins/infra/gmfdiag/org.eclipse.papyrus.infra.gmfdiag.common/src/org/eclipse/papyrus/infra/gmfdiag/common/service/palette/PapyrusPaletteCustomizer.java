...
public class PapyrusPaletteCustomizer extends PaletteCustomizerEx {
	...
	public void save() {
		...
		try {
			...
			Activator.getInstance().logError("Error while saving preferences for palette", e);
		}
		...
		for (int i = idList.size() - 1; i >= 0; i--) {
			...
			if (memento == null) {
				try {
					...
					Activator.getInstance().logError("Error while reading preferences for palette", e);
					return null;
				}
			}
	 ...
	 * <li>when the save button is pressed, the customizations made since the state was stored can be written to the workspace preferences</li>
	 */
